...
#rm(list = ls())
source('Rfunctions18EN.R', echo = F)
options(width = 200)
...
ggdraw(p2)
# ggsave(file = "Results/Figures/Figures/Figure1.tiff",   # The directory you want to save the file in
...
#### Figure 1B: Hospital admissions by the current age and by time after measles.

```{r}
dat.tmp2<-data.total.main
dat.tmp2$time.after.mv<-ifelse(dat.tmp2$period=="before",dat.tmp2$age.at.infection-dat.tmp2$age.at.measles.y,dat.tmp2$time.after.mv)
dat.totaltime <- dat.tmp2 %>% group_by(patid) %>% dplyr::summarise(tstart=min(time.after.mv),tstop=max(time.after.mv),.groups="keep") %>% ungroup() %>% as.data.frame()
dat.event<-dat.tmp2 %>% filter(status==1)
dat.tmp2 %<>% mutate(check=paste0(patid,time.after.mv))
dat.totaltime <- tmerge(dat.totaltime,dat.totaltime,id=patid,tstart=tstart,tstop=tstop)
dat.totaltime <- tmerge(dat.totaltime,dat.event,id=patid,endpt = event(time.after.mv, status))
dat.totaltime %<>% mutate(check=paste0(patid,tstart))
dat.totaltime <-left_join(dat.totaltime,dat.tmp2[,-1],by="check")
dat.totaltime %<>% arrange(age.at.infection,patid,tstart)
dat.totaltime$check<-NULL
dat.totaltime %<>%mutate(id=order(patid)) 

p<-ggplot(data=dat.totaltime,aes(x = time.after.mv, y = patid, group = patid, col = endpt)) +   geom_vline(aes(x = time.after.mv, y = patid, group = patid),col=grey(0.5))+scale_y_discrete(breaks=patid)+
  geom_point(shape = 1)+theme_bw()+geom_point(aes(x = tstart, y = patid),shape=3,colour="blue")+geom_point(aes(x = tstop, y = patid),colour=grey(0.4),shape=1)+ylab("Patients")+xlab("Time since phase 3")


ggplot(data=dat.totaltime)+
geom_segment(aes(x = tstart, y = id, xend = tstop, yend = id),col=grey(0.9))+scale_y_discrete()+geom_point(data=subset(dat.totaltime,status==1),aes(x = time.after.mv, y = id,group=patid),col="red",size=0.2,shape=0)
...
# start_time <- Sys.time()
# fit.gee.main.15<- geeglm(status~ sex + ns(age.at.infection,kn=knots.age,Bo = c(0,15))+I(time.after.mv>=14/365)+
...
# end_time <- Sys.time()
# (time_consumed1<-end_time - start_time)
# save(fit.gee.main.15,file="Results/Final/fit_gee_main_15.Rdata")
```
...
end_time <- Sys.time()
(time_consumed1<-end_time - start_time)
save(fit.gee.main,file="Results/Final/fit_gee_main.Rdata")
```
...
# library(rstanarm)
# 
# #id<-sample(unique(data.total.main$patid),50,replace=F)
# 
# start_time <- Sys.time()
# fit.glmer.main<- stan_glmer(status~ sex + ns(age.at.infection,kn=knots.age,Bo = c(0,age.max))+I(time.after.mv>=14/365)+
#              ns(pmax(time.after.mv-14/365,0),kn=knots.time.af.mv-14/365,Bo = c(0,time.max-14/365))+(1|patid),family=poisson(link = "log"),data = data.total.main,offset=offset_time,QR = FALSE,chains=3)
# end_time <- Sys.time()
# (time_consumed1<-end_time - start_time)
# save(fit.glmer.main,file="Results/Final/fit_glmer_main.Rdata")
```
...
load(file="Results/Final/fit_gee_main.Rdata")
...
```{r message=FALSE, warning=FALSE, include=FALSE}
load(file="Results/Final/fit_gee_main.Rdata")
Time.est<-Time_estimate(fit.gee.main,interval = c(2/52,1.5),knots=knots.time.af.mv,last.knots=5)
```
...
time.after.mv<-c(14/365,21/365,30/365,60/365,90/365,240/365,335/365,1,1.5,2,3,4,5,5.5,6)
l<-length(time.after.mv)
mat<-as.matrix(ns(pmax(time.after.mv-14/365,0),kn=knots.time.af.mv-14/365,Bo = c(0,time.max-14/365))-matrix(rep(ns(0,kn=knots.time.af.mv-14/365,Bo = c(0,time.max-14/365)),each=l),nrow=l))
mat<-cbind(matrix(rep(rep(0,10),l),ncol=10),rep(1,l),mat)

pred<- my.ci.lin(fit.gee.main, mat, Exp=TRUE) 
IRR.tab<-data.frame(time.after.mv=c(14/365,21/365,30/365,60/365,90/365,240/365,335/365,1,1.5,2,3,4,5,5.5,6),age.at.infection=0,offset_time=0)
IRR.tab$pred<-pred[,5]
IRR.tab$plo<-pred[,6]
IRR.tab$phi<-pred[,7]
IRR.tab$time.after.mv <- c("2 weeks","3 weeks","1 month","2 months","3 months","8 months","11 months", "1 year","1.5 year","2 years","3 years","4 years","5 years","5.5 years","6 years")
...
save(IRR.tab,file = "Results/Final/IRR_tab.Rdata")
load(file = "Results/Final/IRR_tab.Rdata")
```
...
p<-ggplot(dat,aes(x=time.after.mv, y=pred)) + geom_line(size=.75,color="#31a354")+
  ...
  scale_y_continuous("IRR relative to before measles",breaks = log(c(1/20,1/8,1/3,1,3,8,50)),labels=c(expression(1/20),expression(1/8),expression(1/3),1,3,8,50))+
  ...
  theme(panel.grid.minor = element_blank(),axis.title.x = element_text(face="bold", colour="black", size=15, margin = margin(t = 20,r = 20,b =20, l = 20)),
        ...
        strip.text=element_text(face = "bold",size=15))  + coord_cartesian(ylim=c(-3,2),xlim=c(0,6)) 
ggsave(file = "Results/Figures/Figures/Figure2B.tiff",   
    width = 7, 
    height = 8, dpi=450) 
p
...
ggsave(file = "Results/Figures/Figures/Figure2A.tiff",   
    width = 15, 
    height = 8, dpi=450) 
p
```
...
ggsave(file = "Results/Figures/Figures/Figure2C.tiff",   
    width = 15, 
    height = 8, dpi=450) 
p
```
...
# start_time <- Sys.time()
# fit.gee.interaction <- geeglm(status~ ns(age.at.infection,kn=knots.age,Bo = c(0,age.max))+I(time.after.mv>=14/365)+
#              ns(pmax(time.after.mv-14/365,0),kn=knots.time.af.mv-14/365,Bo = c(0,time.max-14/365))+ns(age.at.infection,kn=c(1.5),Bo = c(0,age.max)):ns(pmax(time.after.mv-14/365,0),kn=c(0.5)-14/365,Bo = c(0,time.max-14/365)),id=patid,corstr="exchangeable",family=poisson(link = "log"),data = data.total.main,offset=offset_time)
# end_time <- Sys.time()
# (time_consumed1<-end_time - start_time)
# save(fit.gee.interaction,file="Results/fit_gee_interaction.Rdata")
```

```{r tab23, echo=FALSE,results = "asis"}
load(file="Results/fit_gee_interaction.Rdata")
tab<-geeglm.summary(fit.gee.interaction)
knitr::kable(tab)
```

```{r eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, paged.print=TRUE}
# start_time <- Sys.time()
# fit.gee.interaction.sex <- geeglm(status~ sex+ns(age.at.infection,kn=knots.age,Bo = c(0,age.max))+I(time.after.mv>=14/365)+
#              ns(pmax(time.after.mv-14/365,0),kn=knots.time.af.mv-14/365,Bo = c(0,time.max-14/365))+ns(age.at.infection,kn=knots.age,Bo = c(0,age.max)):sex+ns(pmax(time.after.mv-14/365,0),kn=knots.time.af.mv-14/365,Bo = c(0,time.max-14/365)):sex,id=patid,corstr="exchangeable",family=poisson(link = "log"),data = data.total.main,offset=offset_time)
# end_time <- Sys.time()
# (time_consumed1<-end_time - start_time)
# save(fit.gee.interaction.sex,file="Results/fit_gee_interaction_sex.Rdata")
```

```{r tab234, eval=FALSE, include=FALSE}
load(file="Results/fit_gee_interaction_sex.Rdata")
tab<-geeglm.summary(fit.gee.interaction.sex)
knitr::kable(tab)
```
...
```{r include=FALSE}
# load("Results/fit_gee_interaction.Rdata")
# ano_interact<-anova(fit.gee.interaction)
# ano_interact.sex<-anova(fit.gee.interaction.sex)
#ano_interact.sex<-anova(fit.gee.interaction.sex,"ns(age.at.infection, kn = knots.age, Bo = c(0, age.max)):sex+I(time.after.mv >= 14/365):sex +ns(pmax(time.after.mv - 14/365, 0), kn = knots.time.af.mv - 14/365, Bo = c(0, time.max - 14/365)):sex")
...
# save(ano_interact,file="Results/anova_interact_age_time_compare.Rdata")
# save(ano_interact,file="Results/anova_interact_age_time_compare.Rdata")
# save(ano_interact.sex,file="Results/anova_interact_sex_compare.Rdata")


load(file="Results/fit_gee_interaction_overall.Rdata")
...
end_time <- Sys.time()
save(data.total.worse,file = "Data/data_total_worse.Rdata")
...
(age.max<-max(data.total.main$age.at.infection))
(time.max<-max(data.total.main$time.after.mv))
time.max<-9.8
```
...
for (i in 1:num_sim){
  seed_i<-seed_num[i]
  data.total.main.sim[[i]] <- data.transformed(data.raw,option = 2,emigration.rate = 0.0083,seed = seed_i)
}
end_time <- Sys.time()
(time_consumed1<-end_time - start_time)
save(data.total.main.sim,file = "Data/data_total_main_sim.Rdata")
load(file = "Data/data_total_main_sim.Rdata")
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
load(file="Results/dat_sim_result.Rdata")
p<-ggplot(dat,aes(x=time.after.mv, y=pred)) + geom_line(size=.75,color="#31a354")+
  geom_ribbon(data=dat,aes(ymin=plo, ymax=phi, x=time.after.mv), alpha = 0.3,fill="#31a354")+coord_cartesian(ylim=c(-3,4)) +
  scale_y_continuous("IRR relative to before measles",breaks = log(c(1/20,1/8,1/3,1,3,8,50)),labels=c("1/20","1/8","1/3",1,3,8,50))+
  scale_x_continuous("Time after measles infection [years]",limits = c(0,10),breaks=c(0,1:10),labels=c("0",1:10))+
...
pred<- my.ci.lin.mi(fit, mat, Exp=T) 
IRR.tabS1<-data.frame(time.after.mv=c(14/365,21/365,30/365,60/365,90/365,240/365,335/365,1,1.5,2,3,4,5,5.5,6),age.at.infection=0,offset_time=0)
IRR.tabS1$pred<-pred[,5]
IRR.tabS1$plo<-pred[,6]
IRR.tabS1$phi<-pred[,7]
IRR.tabS1$time.after.mv <- c("2 weeks","3 weeks","1 month","2 months","3 months","8 months","11 months", "1 year","1.5 year","2 years","3 years","4 years","5 years","5.5 years","6 years")
...
# end_time <- Sys.time()
# (time_consumed1<-end_time - start_time)
# save(data.total.main.sim_2,file = "Data/data_total_main_sim_2.Rdata")
load(file = "Data/data_total_main_sim_2.Rdata")
...
fit.gee.sim2<-list()
for(i in 1:length(emigration_r)){
  fit.gee.sim2[[i]]<- geeglm(status~ sex+ ns(age.at.infection,kn=knots.age,Bo = c(0,age.max))+I(time.after.mv>=14/365)+
             ns(pmax(time.after.mv-14/365,0),kn=knots.time.af.mv-14/365,Bo = c(0,time.max-14/365)),
...
end_time <- Sys.time()
(time_consumed1<-end_time - start_time)
save(fit.gee.sim2,file="Results/fit_gee_sim2.Rdata")
```



```{r eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
load(file="Results/fit_gee_sim2.Rdata")
time.after.mv<-seq(2/52,6,by=0.01)
l<-length(time.after.mv)
mat<-as.matrix(ns(pmax(time.after.mv-14/365,0),kn=knots.time.af.mv-14/365,Bo = c(0,time.max-14/365))-matrix(rep(ns(0,kn=knots.time.af.mv-14/365,Bo = c(0,time.max-14/365)),each=l),nrow=l))
mat<-cbind(matrix(rep(rep(0,10),l),ncol=10),rep(1,l),mat)
...
for(i in 1:length(emigration_r)){
pred<- my.ci.lin(fit.gee.sim2[[i]], mat, Exp=F) 
...
p<-ggplot(dat,aes(x=time.after.mv, y=pred)) + geom_line(size=.75,color="#31a354")+
  ...
  theme(panel.grid.minor = element_blank(),axis.title.x = element_text(face="bold", colour="black", size=15, margin = margin(t = 20,r = 20,b =20, l = 20)),
        ...
        legend.title = element_blank(),
        legend.key = element_rect(colour = NA),
        strip.text=element_text(face = "bold",size=14))  + coord_cartesian(ylim=c(-3,2),xlim=c(0,6)) 
...
load(file="Results/dat_sim_2_result.Rdata")
emigration_r<-c(0.01,seq(0.1,0.7,by=0.05))
dat$Emigration_Rate<-factor(dat$emigration_rate,levels=c(emigration_r),labels=paste(emigration_r*1000,"per 1000 person-years"))

p<-ggplot(subset(dat,time.after.mv<=6),aes(x=time.after.mv, y=pred,group=Emigration_Rate)) + geom_line(size=.75,aes(color=Emigration_Rate))+
  geom_ribbon(data=subset(dat,time.after.mv<=6& Emigration_Rate=="0.1"),aes(ymin=plo, ymax=phi, x=time.after.mv,fill=Emigration_Rate), alpha = 0.1)+coord_cartesian(ylim=c(-3,4))+scale_color_viridis(discrete = TRUE, option = "D")+
  ...
  theme(panel.grid.minor = element_blank(),axis.title.x = element_text(face="bold", colour="black", size=15, margin = margin(t = 20,r = 20,b =20, l = 20)),
        ...
        legend.key = element_rect(colour = NA),
        strip.text=element_text(face = "bold",size=15))  + coord_cartesian(ylim=c(-3,2),xlim=c(0,6)) 
ggsave(file = "Results/Figures/FigureS3.pdf",   
    width = 7, 
    height = 8, dpi=450) 
p
```
<!-- #### Emigration with extreame rate -->
```{r eval=FALSE, include=FALSE}
data.total.main.sim.extreme <- data.transformed(data.raw,option = 2,emigration.rate = 0.5,seed = 1234)
save(data.total.main.sim.extreme,file="Data/data_total_main_sim_extreme.Rdata")
fit.gee.sim.extreme<- geeglm(status~ ns(age.at.infection,kn=knots.age,Bo = c(0,age.max))+I(time.after.mv>=14/365)+
             ns(pmax(time.after.mv-14/365,0),kn=knots.time.af.mv-14/365,Bo = c(0,time.max-14/365)),
                              id=patid,corstr="exchangeable",family=poisson(link = "log"),data = data.total.main.sim.extreme,offset=offset_time)
save(fit.gee.sim.extreme,file="Results/fit_gee_sim_extreme.Rdata")
```

```{r eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
load(file="Results/fit_gee_sim_extreme.Rdata")
time.after.mv<-seq(2/52,6,by=0.01)
l<-length(time.after.mv)
mat<-as.matrix(ns(pmax(time.after.mv-14/365,0),kn=knots.time.af.mv-14/365,Bo = c(0,time.max-14/365))-matrix(rep(ns(0,kn=knots.time.af.mv-14/365,Bo = c(0,time.max-14/365)),each=l),nrow=l))
mat<-cbind(matrix(rep(rep(0,9),l),ncol=9),rep(1,l),mat)

pred<- my.ci.lin(fit.gee.sim.extreme, mat, Exp=F) 
dat<-data.frame(time.after.mv=seq(2/52,6,by=0.01),age.at.infection=0,offset_time=0)
dat$pred<-pred[,1]
dat$plo<-pred[,5]
dat$phi<-pred[,6]
...
```{r eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
ggplot(dat,aes(x=time.after.mv, y=pred)) + geom_line(size=.75,color="#31a354")+
  geom_ribbon(data=dat,aes(ymin=plo, ymax=phi, x=time.after.mv), alpha = 0.3,fill="#31a354")+coord_cartesian(ylim=c(-3,4)) +
  scale_y_continuous("IRR relative to before measles",breaks = log(c(1/20,1/8,1/3,1,3,8,50)),labels=c("1/20","1/8","1/3",1,3,8,50))+
  scale_x_continuous("Time after measles infection [years]",limits = c(0,10),breaks=c(0,1:10),labels=c("0",1:10))+
  geom_hline(yintercept=0, linetype="dashed", color = "black")+
...
end_time <- Sys.time()
(time_consumed1<-end_time - start_time)
save(fit.gee.sens_13yrs,file="Results/Final/fit_gee_sens_13yrs.Rdata")
```


```{r message=FALSE, warning=FALSE, include=FALSE}
load(file="Results/Final/fit_gee_sens_13yrs.Rdata")
time.after.mv<-seq(2/52,6,by=0.01)
l<-length(time.after.mv)
mat<-as.matrix(ns(pmax(time.after.mv-14/365,0),kn=knots.time.af.mv-14/365,Bo = c(0,time.max-14/365))-matrix(rep(ns(0,kn=knots.time.af.mv-14/365,Bo = c(0,time.max-14/365)),each=l),nrow=l))
mat<-cbind(matrix(rep(rep(0,10),l),ncol=10),rep(1,l),mat)
...
ggsave(file = "Results/Figures/Supplementary Figures/S5_Figure.pdf",   
    width = 10, 
    height = 9) 
p
...
p<-ggplot(subset(dat,Year==2015),aes(x = Location, y = Value,color=Age))+labs(fill="Countries")+geom_point(size=1.5,position = position_dodge(width=0.3))+scale_color_manual(values=c("#f03b20","#3182bd"))+facet_wrap(~Cause_of_death,scales="free_y",nrow=2) + geom_errorbar(aes(x = Location,ymin = Lower_bound,ymax = Upper_bound),width=0.5,position = position_dodge(width=0.3))+ylab("Probability of death due to infectious diseases ")+theme_bw() +
    theme(axis.title.x = element_text(face="bold", colour="black", size=15, margin = margin(t = 20,r = 20,b =20, l = 20)),
        ...
        strip.text=element_text(face = "bold",size=15),
        plot.title=element_text(family='', face='bold', colour='black', size=15))
ggsave(file = "Results/Figures/Supplementary Figures/S6_Figure.pdf",   
    width = 10, 
    height = 9) 
p
```
...
save(data.total.main.sim_test,file = "Data/data.total.main.sim_test.Rdata")
load(file = "Data/data.total.main.sim_test.Rdata")
