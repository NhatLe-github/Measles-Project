...
MCMC_estimate<-function(fit,interval,knots,last.knots){
  ...
  data<-draws_array[,,names(fixef(fit))]
  data.new<-rbind(data[,1,],data[,2,],data[,3,])
  mcmc_coef.time.effect<-as.data.frame(data.new[,names(fixef(fit))[-c(1:9)]])
  T<-sapply(1:nrow(mcmc_coef.time.effect), function(i){
    extend<-ifelse(interval[1]>0.5,"upX","downX")
    e <- try( d <-uniroot(func2, c(interval[1], interval[2]), tol = 1e-10,extendInt=extend, knots=knots,coef=t(unname(mcmc_coef.time.effect[i,])),last.knots=last.knots))
...
glmer.summary<- function(obj){
  # Simple summary for a geeglm object
  s<-nrow(summary(obj)$coefficients)
  dat<-data.frame()
  if(s>2){
    for(i in 1:s){
      position<-c(rep(0,(i-1)),1,rep(0,(s-i)))
      tmp<-esticon(obj,position)
      dat.tmp<-data.frame(Estimate=tmp[,2],se=tmp[,3],exp.Estimate=exp(tmp[,2]),Lower.CI=exp(tmp[,7]),Upper.CI=exp(tmp[,8]))
      dat<-rbind(dat,dat.tmp)
    }
  }else{
    position<-c(1,0)
  ...
  dat$covariate<-rownames(summary(obj)$coefficients)
  dat<-dat[,c(6,1:5)]
  dat$p.value<-format.pval(2*(1-pnorm(abs(dat$Estimate/dat$se),0,1)))
  dat<-dat[-1,]
  rownames(dat)<-NULL
  return(dat)
}
...
pred.gee.diff_msm<-function(obj,newdata){
  library(msm)
  dat<-newdata
  
  for(i in 1:nrow(dat2)){
    mm<-model.matrix(~sex + ns(age.at.infection,kn=knots.age,Bo = c(0,age.max))+I(time.after.mv>=14/365)+
                       ns(pmax(time.after.mv-14/365,0),kn=knots.time.af.mv-14/365,Bo = c(0,last.knots-14/365)),data=dat[i,])
    a1<-mm[1]
    a2<-mm[2]
    a3<-mm[3]
    a4<-mm[4]
    ...
    a19<-mm[19]
    a20<-mm[20]
    pse1<-deltamethod(~exp(a1*x1+a2*x2+a3*x3+a4*x4+a5*x5+a6*x6+a7*x7+a8*x8+a9*x9+a10*x10+
                             a11*x11+a12*x12+a13*x13+a14*x14+a15*x15+a16*x16+a17*x17+a18*x18+a19*x19+a20*x20)-
                        exp(a1*x1+a2*x2+a3*x3+a4*x4+a5*x5+a6*x6+a7*x7+a8*x8+a9*x9+a10*x10),
                      mean=obj$coefficients, cov=obj$geese$vbeta)
    dat$absolute_diff[i]<-exp(mm%*%obj$coefficients)- exp(mm[1,1:10]%*%obj$coefficients[1:10])
    dat$absolute_diff_lo[i]<-dat$absolute_diff[i]-qnorm(0.975,0,1)*pse1
    dat$absolute_diff_high[i]<-dat$absolute_diff[i]+qnorm(0.975,0,1)*pse1
  }
  return(dat)
}

fit.Poisson_mixed_model<-function(formula=status~ ns(age.at.infection,kn=knots.age,Bo = c(0,age.max))+I(time.after.mv>=14/365)+
                                    ns(pmax(time.after.mv-14/365,0),kn=knots.time.af.mv-14/365,Bo = c(0,last.knots-14/365)),data,emigration_rate=0.0083,n.adapt=3000,n.iter=4000,n.chain=3,max_treedepth=14,adapt_delta = 0.95,seed=10){
  set.seed(seed)
  N<-nrow(data)
  patient<-as.integer(factor(data$patid))
  n_patient<-length(patient)
  y<-data$status
  X<-model.matrix(formula,data)
  p_X<-ncol(X)
  time_interval <-data$time.interval
  index_prob <- as.integer(data$index_prob)
  emigration_rate<-emigration_rate
  Measles.data<-c("N","n_patient","patient","time_interval","emigration_rate","y","X","p_X","index_prob")
  fit <- stan(file = 'Poisson_fit.stan', data = Measles.data,warmup=n.adapt,thin=10,
              iter = n.iter, chains = n.chain,pars=c("b"),control=list(max_treedepth=max_treedepth,adapt_delta = adapt_delta))
